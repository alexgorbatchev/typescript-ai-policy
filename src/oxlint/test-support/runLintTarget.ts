...
export function runLintTarget(fixtureRepositoryPath: string): LintTargetResult {
  ...
  let oxlintJsonOutput: OxlintJsonOutput;

  try {
    oxlintJsonOutput = JSON.parse(jsonText) as OxlintJsonOutput;
  } catch {
    throw new Error(`Could not parse Oxlint JSON output from lint-target output:\n${rawOutput}`);
  }
...
export function runLintTargetWithConfigPath(fixtureRepositoryPath: string, configPath: string): LintTargetResult {
  ...
  try {
    oxlintJsonOutput = JSON.parse(normalizedStdout) as OxlintJsonOutput;
  } catch {
    throw new Error(`Could not parse Oxlint JSON output in direct-config lint-target output:\n${rawOutput}`);
  }
