import { getFilenameWithoutExtension, isExemptSupportBasename, readDeclarationIdentifierNames } from "./helpers.js";
...
function readExpectedHookNameFromFilename(filename) {
...
function readKebabCaseHookFilename(hookName) {
  return `${hookName.replaceAll(/([a-z0-9])([A-Z])/gu, "$1-$2").toLowerCase()}.ts`;
}

const hookFileNamingConventionRule = {
  meta: {
