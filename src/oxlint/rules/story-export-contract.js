import { getStorySourceBaseName, isPascalCase, unwrapExpression, unwrapTypeScriptExpression } from "./helpers.js";

function readDefaultExportDeclaration(program) {
  return program.body.find((statement) => statement.type === "ExportDefaultDeclaration") ?? null;
}

function readExpectedComponentNameFromStoryFilename(filename) {
...
function isStoryTypeReference(typeAnnotation) {
  return (
...
function readStoryAssertionTypeAnnotation(expression) {
...
function readStoryObjectExpression(expression) {
...
function hasPlayProperty(expression) {
...
function readTopLevelStoryCandidateEntries(program) {
  const entriesByName = new Map();

  function upsertEntry(variableDeclaration, declarator) {
  ...
  function recordVariableDeclaration(variableDeclaration, isDirectExport) {
...
function isStoryCandidateEntry(entry, metaBindingName) {
      ...
      Program(node) {
        ...
        if (exportedStoryEntries.length === 1) {
          ...
          const hasValidSingleStoryExportShape =
            ...
            singleStoryEntry.exports[0]?.kind === "specifier" &&
            singleStoryEntry.exports[0].localName === "Default" &&
            singleStoryEntry.exports[0].exportedName === expectedComponentName;

          if (!hasValidSingleStoryExportShape) {
            context.report({
              node: singleStoryEntry.exports[0]?.node ?? singleStoryEntry.declarator,
        ...
        exportedStoryEntries.forEach((storyEntry) => {
          const hasValidMultiStoryExportShape =
            ...
            storyEntry.exports[0]?.kind === "declaration" &&
            storyEntry.exports[0].exportedName === storyEntry.name;
          ...
          context.report({
            node: storyEntry.exports[0]?.node ?? storyEntry.declarator,
